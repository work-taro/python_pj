...
def product_template_conn_db():
    ...
    try:
        ...
        if conn.connect:
            ...
            product_template_condition = ["|",["categ_id","ilike","สินค้าสั่งผลิต / ม่านรังผึ้ง"],["categ_id","ilike","สินค้าสั่งผลิต / ม่านม้วน"]]
                ...
                if duplicates:
                    ...
                    for id_attribute_line in loop_line:
                        ...
                        for attr in attr_line:
                            ...
                            product_template_id = attr['product_tmpl_id'][0] if isinstance(attr['product_tmpl_id'],
                                                                                           list) else None
                            ...
                            try:
                                ...
                                """
                                cursor.execute(insert_sql,
                                               (attr_line_id, product_template_id, attr_id, attr_value_id,
                                                default_value_id, custom, now, now))
                            except Exception as e:
                                print(f"Exception is {e}")
            conn.commit()
            cursor.close()
    ...
    try:
        ...
        if conn.connect:
            ...
            for product_attribute_value in product_attribute_value_template:
                ...
                attribute_id = product_attribute_value['attribute_id'][0] if isinstance(
                    product_attribute_value['attribute_id'], list) else None
                ...
                for multi_combo in attr_value_tem_s:
                    # print(multi_combo)
                    product_variants_id = multi_combo['product_id'][0] if isinstance(multi_combo['product_id'],
                                                                                     list) else None
    ...
    try:
        ...
        if conn.connect:
            ...
            for default in attr_default_template:
                ...
                attribute_value_id = default['attribute_value_id'][0] if isinstance(default['attribute_value_id'],
                                                                                    list) else None
                ...
                insert_sql = """
                                ...
                                """
                cursor.execute(insert_sql,
                               (default_id, product_id, attribute_value_id, default_attribute_value_id, now, now))
            ...
            for formula in formula_template:
                ...
                try:
                    insert_sql = """
                            ...
                            """
                    cursor.execute(insert_sql, (
                        formula_id, formula_name, price_based, uom, formula_type, dimension, code, now, now))
                except Exception as e:
                    print(f"Exception is {e}")
...
def product_pricelist_item():
    ...
    try:
        ...
        if conn.connect:
            ...
            cursor.execute(alter)
            pricelist_item_condition = ["|", ["product_tmpl_id.categ_id", "ilike", "สินค้าสั่งผลิต / "],
                                        ["product_tmpl_id.categ_id", "ilike", "วัตถุดิบ / "]]
            ...
            for pricelist in pricelist_template:
                ...
                product_name = pricelist['product_tmpl_id'][1] if isinstance(pricelist['product_tmpl_id'],
                                                                             list) else None
                ...
                try:
                    ...
                    """
                    cursor.execute(insert_sql, (
                        pricelist_line_id, product_id, product_name, pricelist_id, pricelist_name, compute_price,
                        fixed_price,
    ...
    try:
        ...
        if conn.connect:
            ...
            pricelist_type_template = models.execute_kw(db, uid, password,
                                                        ...
                                                        [pricelist_type_condition], {
                                                            'fields': ['id', 'name', 'discount_policy',
                                                                       'display_pricelist']})
            for pricelist in pricelist_type_template:
                ...
                try:
                    ...
                    """
                    cursor.execute(insert_sql,
                                   (pricelist_id, pricelist_name, discount_policy, display_pricelist, now, now))
                except Exception as e:
                    print(f"Exception is {e}")
                # print(attribute_line_ids)
            conn.commit()
    ...
    try:
        ...
        if conn.connect:
            ...
            while offset < total_records:
                # ดึงข้อมูลทีละ batch
                variant_value_condition = ["|",["product_tmpl_id.categ_id","=","สินค้าสั่งผลิต / ม่านรังผึ้ง"],["product_tmpl_id.categ_id","=","สินค้าสั่งผลิต / ม่านม้วน"]]
                variant_value_template = models.execute_kw(db, uid, password,
                                                           'product.template.attribute.value', 'search_read',
                                                           [variant_value_condition], {
                                                               'fields': ['id', 'product_tmpl_id', 'categ_id',
                                                                          'attribute_line_id',
                                                                          'attribute_id', 'product_attribute_value_id',
                                                                          'attribute_size_extra', 'price_extra',
                                                                          'size_based_on',
                                                                          'size_based_on_uom_id', 'cash_price',
                                                                          'credit_price',
                                                                          'modern_trade', 'customer_price',
                                                                          'online_price',
                                                                          'discount_cash_price',
                                                                          'discount_credit_price',
                                                                          'discount_modern_trade',
                ...
                for variant in variant_value_template:
                    ...
                    product_name = variant['product_tmpl_id'][1] if isinstance(variant['product_tmpl_id'],
                                                                               list) else None
                    product_cate_id = variant['categ_id'][0] if isinstance(variant['categ_id'], list) else None
                    attribute_line_id = variant['attribute_line_id'][0] if isinstance(variant['attribute_line_id'],
                                                                                      list) else None
                    attribute_id = variant['attribute_id'][0] if isinstance(variant['attribute_line_id'],
                                                                            list) else None
                    attribute_value_id = variant['product_attribute_value_id'][0] if isinstance(
                        variant['product_attribute_value_id'], list) else None
                    attribute_size_extra = variant['attribute_size_extra'] if variant[
                                                                                  'attribute_size_extra'] is not None else None
                    ...
                    uom = variant['size_based_on_uom_id'][1] if isinstance(variant['size_based_on_uom_id'],
                                                                           list) else None
                    ...
                    discount_cash_price = variant['discount_cash_price'] if variant[
                                                                                'discount_cash_price'] is not None else None
                    discount_credit_price = variant['discount_credit_price'] if variant[
                                                                                    'discount_credit_price'] is not None else None
                    discount_modern_trade_price = variant['discount_modern_trade'] if variant[
                                                                                          'discount_modern_trade'] is not None else None
                    discount_customer_price = variant['discount_customer_price'] if variant[
                                                                                        'discount_customer_price'] is not None else None
                    discount_online_price = variant['discount_online_price'] if variant[
                                                                                    'discount_online_price'] is not None else None
                    ...
                    try:
                        ...
                        """
                        cursor.execute(insert_sql,
                                       (varaint_line_id, product_id, product_name, product_cate_id, attribute_line_id,
                                        ...
                                        online_price, discount_cash_price, discount_credit_price,
                                        discount_modern_trade_price,
...
def process():
    ...
    print(f"total processing : {time.time() - ts} second")

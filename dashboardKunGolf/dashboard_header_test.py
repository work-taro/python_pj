...
def module_stcrd(dbf_file_path_stcrd, df_purchase_date):
    ...
    for record in table:
        ...
        if docnum_prefix in ['IV', 'OL', 'OU', 'PD', 'OC', 'OW', 'OO']:
            if stkcod not in latest_withdraw_date or doc_date > latest_withdraw_date[stkcod]:
                latest_withdraw_date[stkcod] = doc_date
        ...
        if docnum_prefix in ['HI', 'RI', 'RR', 'HP', 'PI', 'JI', 'LL']:
            if stkcod not in earliest_receive_date:
                earliest_receive_date[stkcod] = doc_date
            else:
                if doc_date < earliest_receive_date[stkcod]:
                    earliest_receive_date[stkcod] = doc_date
    ...
    df_withdraw = pd.DataFrame(list(latest_withdraw_date.items()), columns=['STKCOD', 'latest_withdraw_date'])
    df_receive = pd.DataFrame(list(earliest_receive_date.items()), columns=['STKCOD', 'earliest_receive_date'])
    ...
    df_purchase_fill['earliest_receive_date'] = pd.to_datetime(df_purchase_fill['earliest_receive_date'], errors='coerce')
...
def module_stmas(dbf_file_path_stmas):
    ...
    for record in data:
        ...
        try:
            ...
            lpur_dat = None
        stmas_record.append([str(record.STKCOD), float(record.TOTBAL), float(record.UNITPR), float(record.TOTVAL), lsel_dat, lpur_dat])
...
def calculate_withdrawn_and_bought_in_values(df_stcrd, df_stkcod):
    ...
    if not filtered_df.empty:
        ...
        withdrawn_summary = withdrawn_summary.reindex(columns=['STKCOD', 'withdrawn_last_month', 'withdrawn_this_month'], fill_value=0)
        ...
        summary = pd.DataFrame(columns=['STKCOD', 'withdrawn_last_month', 'withdrawn_this_month', 'total_bought_in_qty'])
...
def insert_product_data(df):
    ...
    try:
        ############################ INSERT MYSQL ############################
        # ชื่อตาราง
        table_name = 'dashboard_product_header_copy1'

        # เชื่อมต่อฐานข้อมูล MySQL
        engine = create_engine("mysql+pymysql://{user}:{pw}@{host}/{db}?charset=utf8mb4".format(user=db_username, pw=db_password, host=db_host, db=db_database))
        ...
        df = df[['stkcod', 'stkdes', 'barcode', 'category', 'group', 'unit', 'total_bought_in_qty', 'remaining_amount', 'cost', 'residual_value',
                 'withdrawn_last_month', 'withdrawn_this_month', 'latest_withdraw_date',  'earliest_receive_date','owner_emp_id', 'created_at', 'updated_at']]
        ...
        # เปลี่ยนชื่อคอลัมน์
        df = df.set_axis(['stkcod', 'stkdes', 'barcod', 'category', 'group', 'unit', 'total_bought_in_amount', 'remaining_amount', 'cost', 'residual_value',
                 'withdrawn_last_month', 'withdrawn_this_month', 'latest_date_export', 'earliest_date_import', 'owner', 'created_at', 'updated_at'], axis=1)
...
def task():
    ...
    df = pd.merge(df_product, df_stmas[['STKCOD', 'remaining_amount', 'cost', 'residual_value']], left_on='stkcod',
                  right_on='STKCOD', how='left')
    ...
    df_stcrd_grouped = df_stcrd.groupby('STKCOD').agg({
        'latest_withdraw_date': 'max',  # เอาวันที่ล่าสุดที่เบิกออก
        'earliest_receive_date': 'min'  # เอาวันที่รับเข้าแรกสุด
    }).reset_index()

    df = pd.merge(df, df_stcrd_grouped, left_on='stkcod', right_on='STKCOD', how='left')

    # แปลง NaN เป็น 0
    columns_to_fill = ['remaining_amount', 'cost', 'residual_value', 'withdrawn_last_month', 'withdrawn_this_month', 'total_bought_in_qty']

...
def process():
    ...
    df = df[(df['DOCDAT'] >= pd.Timestamp('2024-01-01')) & (df['DOCDAT'] <= pd.Timestamp.today())]
    ...
    print(f"📦 Total new records prepared: {len(insert_data)}")
            ...
            conn.execute(
                text("""
                    INSERT INTO artrn_invoice (
                        ...
                        amount, discount, total, credit_total, net_total,
